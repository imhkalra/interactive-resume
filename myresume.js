...
var ct1,ct2,ct3,ct4;
var bulletTime = 0;
var criticalThinking, prodStrategy, userResearch, agileDev;
var birds;
var bubble;
...
function preload(){
	...
	game.load.atlasJSONHash('himanshu','assets/hero_running.png','assets/hero_running.json');
	...
	game.load.image('confluence','assets/confluence.png');
	game.load.image('jira','assets/jira.png');
	game.load.image('clevertap','assets/clevertap.png');
	game.load.image('mixpanel','assets/mixpanel.png');
	...
	game.load.image('metabase','assets/metabase.png');
...
//	game.load.onLoadStart.add(loadStart, this);
  ...
  //  game.stage.scale.startFullScreen();
   ...
   //game.canvas.addEventListener('touchenter', function(){ console.log("You touched here!!");}, false);
	...
	userResearch = game.add.text(5050,h-320-50,'User\n Research',style_roboto);
	prodStrategy=	game.add.text(5250,h-430-60,'Product\n Strategy',style_white2);
	agileDev = game.add.text(5430,h-450-50,'Agile\n Development',style_white2);
	criticalThinking = game.add.text(5660,h-460-50,'Critical\n Thinking',style_white2);

	
	userResearch.alpha = 0.2;
	prodStrategy.alpha = 0.2;
	agileDev.alpha = 0.2;
	criticalThinking.alpha = 0.2;
	game.add.text(5200,h-140,'PRODUCT MANAGEMENT SKILLS',style_roboto);
	...
	*/
	...
	codetext = game.add.text(6550,h-550,'T     O L S =>',mustard_style);
   ...
   //Add Explosion object pool
	...
	game.add.text(9740,h-260,"Experience",{font:"20px Arial Black"});


	game.add.sprite(11800,h-440,'level');
	game.add.text(11870,h-340,"Level 4");
	game.add.text(11940,h-260,"Achievements",{font:"18px Arial Black"});
	...
	game.add.text(10035,h-210,"E-cell Member at\n DTU conducting multiple\n workshops and events",style_exS);
	game.add.text(10350,h-220,"Founder and \nFrontend Engineer,\n 7Confirmations",style_exS);
	game.add.text(10650,h-230,"Entrepreneur in \nResidence - Product\n at NirogStreet",style_ex); 
	game.add.text(10950,h-260,"Promoted to Product Manager\n at NirogStreet \n(Apr 2021 - Apr 2023)",style_ex);
	game.add.text(11360,h-280,"Growth Manager - Product\n at Coding Ninjas \n (May 2023 - Present)",style_ex);
	...
	*/
	...
	game.add.text(4300,h-120,"  Scored 8.8 CGPA in university.",style_roboto);
	
	//Add bubble
	
	bubble = game.add.sprite(9100,h+450,'psbubble');
	game.add.text(8900,h+500,"Designing Skills - Figma, Canva, Basic Photoshop",style_roboto);
	...
	//Add hero 
	hero = game.add.sprite(100,-200,'himanshu');
...
function collisionHandler(bullet,tile){
    ...
    if(explosion_count==1){
		var css = game.add.sprite(bv,h-400,'jira');
	...
	var html= game.add.sprite(bv+1*inc,h-500,'mixpanel');
	...
	var js= game.add.sprite(bv+2*inc,h-500,'clevertap');
	...
	var py= game.add.sprite(bv+3*inc,h-500,'confluence');
	...
	*/
	...
	var andro= game.add.sprite(bv+4*inc,h-500,'metabase');
...
function fadeText(){
	 ...
	 game.add.tween(agileDev).to( { alpha: 1 }, 2000, Phaser.Easing.Linear.None, true).start();
	 game.add.tween(criticalThinking).to( { alpha: 1 }, 2000, Phaser.Easing.Linear.None, true).start();
	 game.add.tween(prodStrategy).to( { alpha: 1 }, 2000, Phaser.Easing.Linear.None, true).start();
	 game.add.tween(userResearch).to( { alpha: 1 }, 2000, Phaser.Easing.Linear.None, true).start();
}
